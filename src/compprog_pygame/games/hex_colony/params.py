...
# -------------------------------------------------------------------
#  RIVAL AI COLONY  (the "Other Colony" enemy faction)
#  Tunables for the AI rival faction that races the player to the
#  rocket. Spawns on every world. See rival_colony.py for details.
# -------------------------------------------------------------------
...
# Tier index at which the rival begins assembling their rocket.
# Match the player's ROCKET_SILO unlock tier so the race feels fair.
RIVAL_LAUNCH_TIER: int = 7

# Real-time seconds (at 1x sim speed) needed to advance from tier 0 to
# tier 1.  Each subsequent tier scales by RIVAL_TIER_TIME_SCALE.
RIVAL_TIER_TIME_BASE: float = 90.0
RIVAL_TIER_TIME_SCALE: float = 1.35

# Real-time seconds at 1x speed for the rival's rocket bar to fill
# once it starts ticking.  At 5x speed this completes in ~36s.
RIVAL_ROCKET_TIME: float = 180.0

# Real-time seconds between hostile raid attempts (sim seconds).
RIVAL_RAID_INTERVAL: float = 35.0
# Fraction of a building's stored stockpile a single theft removes.
RIVAL_RAID_STEAL_FRACTION: float = 0.4
# Sim seconds a sabotage raid keeps the target building disabled.
RIVAL_RAID_DISABLE_TIME: float = 12.0

# Real-time seconds between unsolicited gifts when relations are
# FRIENDLY or ALLIED.
RIVAL_GIFT_INTERVAL: float = 60.0

# How often (sim seconds) the rival places a new HOUSE outward from
# its TRIBAL_CAMP, and the cap on how many HOUSEs they may place.
RIVAL_BUILD_INTERVAL: float = 18.0
RIVAL_MAX_HOUSES: int = 24
# Maximum hex distance from the TRIBAL_CAMP at which a HOUSE may be
# placed.  Keeps the rival visually grouped.
RIVAL_BUILD_RADIUS: int = 5

# Display-only: how many "people" each visible building represents in
# the diplomacy panel summary.  Not a real simulated head-count.
RIVAL_POP_PER_BUILDING: int = 3

# Bounded length of the recent-events log shown to the player.
RIVAL_LOG_LIMIT: int = 24

# Resource the player must pay to end a war declared by either side.
RIVAL_PEACE_TRIBUTE_RESOURCE: str = "FOOD"
RIVAL_PEACE_TRIBUTE_AMOUNT: int = 200

# Bootstrap stockpile so the rival can immediately gift / trade.
RIVAL_START_RESOURCES: dict[str, int] = {
    "WOOD": 200,
    "FIBER": 100,
    "STONE": 200,
    "FOOD": 150,
